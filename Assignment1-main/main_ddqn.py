...
BASE_OUTPUT_DIR = "outputs/part_ddqn"
STATS_PATH = "stats.md"
ALGORITHM_NAME = "Double DQN"

parser = argparse.ArgumentParser(description="Train a Double DQN on LunarLander-v3")
parser.add_argument(
    "--device",
...
class DDQNAgent:
    ...
    def train_step(self):
        ...
        with torch.no_grad():
            # Double DQN:
            # 1. use the online network to choose the best next action
            next_actions = self.q_network(next_states).argmax(dim=1, keepdim=True)
            # 2. use the target network to evaluate that chosen action
...
for episode in range(num_episodes):
    ...
    while not done:
        # Double DQN still uses epsilon-greedy action selection for exploration.
    ...
    if (episode + 1) % CHECKPOINT_FREQ == 0:
        checkpoint_path = os.path.join(CHECKPOINT_DIR, f"ddqn_episode_{episode + 1}.pt")
...
final_checkpoint = os.path.join(CHECKPOINT_DIR, "ddqn_final.pt")
...
print()
print("Evaluation over 100 no-exploration (Double DQN learned policy) episodes")
